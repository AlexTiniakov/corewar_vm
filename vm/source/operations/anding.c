...
/*   Updated: 2018/09/28 20:35:16 by dskrypny         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	extended_anding(unsigned int arg[3], int codage)
{
	int		shift;
	int		i;
	int		result;
...
void		anding(t_core *core, t_fork **fork)
{
	...
	if (core->extended)
		ft_printf("%-4d||%5{grn}s||", (*fork)->curr_point, "and");
	if (!check_codage(codage, 6))
	{
		shift += 1 + select_params(core, fork, codage, arg);
		if (core->extended)
			extended_anding(arg, codage);
